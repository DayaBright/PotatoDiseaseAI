...
class DateUtilsUnitTest {
    ...
    // Importación implícita de com.tesis.potatodiseaseai.utils.DateUtils por su uso en el test.
    ...
    fun `PT-U07b — timestamp 0 (epoch) se formatea sin error`() {
        val formatted = DateUtils.formatTimestamp(0L)
    ...
    fun `PT-U07c — diferencia menor a 1 minuto muestra 'Hace un momento'`() {
        ...
        assertEquals("Hace un momento", DateUtils.getRelativeTime(timestamp))
    }
    ...
    fun `PT-U07d — diferencia de 5 minutos muestra 'Hace 5 min'`() {
        ...
        assertEquals("Hace 5 min", DateUtils.getRelativeTime(timestamp))
    }
    ...
    fun `PT-U07e — diferencia de 3 horas muestra 'Hace 3h'`() {
        ...
        assertEquals("Hace 3h", DateUtils.getRelativeTime(timestamp))
    }
    ...
    fun `PT-U07f — diferencia de 2 dias muestra 'Hace 2d'`() {
        ...
        assertEquals("Hace 2d", DateUtils.getRelativeTime(timestamp))
    }
    ...
    fun `PT-U07g — diferencia mayor a 7 dias muestra fecha completa`() {
        ...
        val result = DateUtils.getRelativeTime(timestamp)
